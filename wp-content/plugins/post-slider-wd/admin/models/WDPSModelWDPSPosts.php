...
class WDPSModelWDPSPosts {
  ...
  $custom_fields_name = array();
    ...
    for ($i = 0; $i < count($posts); $i++) {
      ...
      $post = $posts[$i];
      $jsonik = array();
      if (!post_password_required($post->ID) && has_post_thumbnail($post->ID) && (!$search_value || (stristr($post->post_title, $search_value) !== FALSE))) {
        $counter++;
   ...
   return array($row, $counter, $page_limit,$json,$posts_fildes_names);
  }
