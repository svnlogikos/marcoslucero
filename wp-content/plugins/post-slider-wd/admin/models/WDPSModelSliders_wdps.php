...
class WDPSModelSliders_wdps {
  ...
  public function get_slides_count($slider_id) {
    global $wpdb;
      $count = $wpdb->get_var($wpdb->prepare("SELECT COUNT(id) FROM " . $wpdb->prefix . "wdpsslide WHERE slider_id='%d' AND image_url<>''", $slider_id));
    return $count;
  }
    ...
    if (!$rows) {
      ...
      foreach ($rows as $row) {
        if ($row->type == 'image') {
          ...
          $row->thumb_url = $row->thumb_url ? $row->thumb_url : WD_PS_URL . '/images/no-image.png';
        }
      }
    }
    return $rows;
